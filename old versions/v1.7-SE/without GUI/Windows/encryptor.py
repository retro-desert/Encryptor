...
version_data = script_directory + "\\version.txt"
version_server_file = script_directory + "\\version_server.txt"
...
def twofish_encrypt():
    ...
    print(twofish_encryption.test)
    d = input("256 bit password will be generated if you enter nothing"
              "\nInput Password:\n")
...
def update(s=1):
    h = open(version_data, "wb")
    pickle.dump(version, h)
    h.close()
    try:
        url = "https://raw.githubusercontent.com/retro-desert/Encryptor/master/version_server.txt"
        wget.download(url, version_server_file)

        o = open(version_server_file, "rb")
        storedlist = pickle.load(o)
        o.close()

        o1 = open(version_data, "rb")
        storedlist1 = pickle.load(o1)
        o1.close()

        if storedlist == storedlist1:
            if s == 1:
                print("\nVersion is up to date!")
        else:
            raise Exception("New version available!")
    except OSError:
        print("Internet is disabled!")
    except Exception:
        print("\nNew version available!"
              "Download: https://github.com/retro-desert/Encryptor/releases")
